...
export class Container {
    ...
    private withChain(chain: Identifier<unknown>[]): Container {
        return new Container(this.registry, this.root ?? this, chain, this.cache);
    }

    private findRegistration<T>(identifier: Identifier<T>): Registration<T> | undefined {
        return (
            this.registry.get(identifier as Identifier<unknown>) ?? this.root?.registry.get(identifier as Identifier<unknown>)
        ) as Registration<T> | undefined;
    }

    public hasRegistration<T>(identifier: Identifier<T>): boolean {
        return this.registry.has(identifier as Identifier<T>);
    }
}
